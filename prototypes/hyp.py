...
# es - datos de entrada;
# na - datos objetivos
esp = np.random.uniform(
    0, 15, size=[INSTANCES, NODES_INPUT]).astype(np.float32)
...
# h(x) = x*w + bias
hidden_layer1_output = tf.add(
    tf.matmul(esp, layer1["W1"]), layer1["b1"], name="hidden1")
...
# h(x) = x*w + bias
hidden_layer2_output = tf.add(
    tf.matmul(hidden_layer1_output, layer2["W2"]), layer2["b2"], name="hidden2")
# Función de activación usando ReLU
hidden_layer2_output = tf.nn.relu(hidden_layer2_output, name="actfhidden2")


# calcular la salida la NN
output = tf.add(tf.matmul(hidden_layer2_output,
                          output_layer["W2"]), output_layer["b2"], name="output")
...
optimiser = tf.train.GradientDescentOptimizer(
    learning_rate=LEARNING_RATE, name="GradientDescent").minimize(loss, name="loss")
...
Change the weights initialization to tf.truncated_normal(shape, stddev=0.1) and 
bias to tf.constant(0.1, shape=shape). Got 98% accuracy. :D
'''
