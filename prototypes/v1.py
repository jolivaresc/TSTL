...
W1 = tf.Variable(tf.truncated_normal([NODES_INPUT, NODES_H1], stddev=0.1, dtype=tf.float64), name="W1")
# Bias.
...
W_nah = tf.Variable(tf.truncated_normal([NODES_H1, NODES_OUTPUT], stddev=0.1, dtype=tf.float64), name="W_nah")
b_nah = tf.Variable(tf.constant(0.1, shape=[NODES_OUTPUT], dtype=tf.float64), name="b_nah")

nah_predicted = tf.nn.xw_plus_b(fc1,W_nah,b_nah)
...
#L2 = tf.nn.l2_loss(W1) + tf.nn.l2_loss(nah_predicted)
#loss = tf.reduce_mean(loss + 0.01 * L2)
